...
public class PasswordResetTokenServiceTests
{
    ...
    {
        _sut = new PasswordResetTokenService(new EphemeralDataProtectionProvider());
    }
    ...
    {
        var otherService = new PasswordResetTokenService(new EphemeralDataProtectionProvider());
    ...
    [Fact]
    public void Unprotect_EmailConfirmationToken_ThrowsCryptographicException()
    {
        var emailService = new EmailConfirmationTokenService(new EphemeralDataProtectionProvider());
        var token = emailService.Protect("userId", "identity-token");
